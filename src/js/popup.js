...
function onError(error) {
  DEBUG && console.error("[popup]", `${error}`);
}
...
async function getCurrentTab() {
  ...
  let tab = tabs[0];
  DEBUG && console.assert(typeof tab.url == 'string', 'tab.url should be a string');
...
function sendBgMessage(message) {
  ...
  msg.then( response => {
    DEBUG && console.log("[popup/sendBgMessage] response:", response);
  });
}

async function downloadHashes(addToStorage) {
  DEBUG && console.assert(typeof addToStorage == 'boolean', 'addToStorage should be a boolean');
  ...
  DEBUG && console.log('[popup/download] current tab id is', tab.id);
  ...
  domRequestMsg.then( (resp) => {
    ...
    DEBUG && console.log("[popup/domRequest] returned data:", data);
    ...
    DEBUG && console.log("[popup/exportHashes]", "filename:", name, "url:", url);
    ...
    if (addToStorage == false) {
      ...
      msgSendImport.then( msgResp => {
        DEBUG && console.log("[popup/msgSendImport] response from background:", msgResp.response);
